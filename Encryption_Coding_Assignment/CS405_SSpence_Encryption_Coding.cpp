...
std::string read_file(const std::string& filename)
{
    ...
    // If the file is open
    if (read_text) {
        std::stringstream string_stream;    // Initialize a string stream
        string_stream << read_text.rdbuf(); // Read the file
        file_text = string_stream.str();    // Put the contents of the file back into the std::string
    }
    else 
    {
        std::cerr << "The input file was not opened." << std::endl;
    }
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    char format[64];     // Create char array for output
    // tm structure for time values
    struct tm format_time;
    localtime_s(&format_time, &time_now);
    // Format time output
    strftime(format, sizeof(format), "%Y-%m-%d", &format_time);
    // Print time to output file in format yyyy-mm-dd
    output_file << format << std::endl;

    //  Line 3: key used
    output_file << "The key used was: " << key << std::endl;
