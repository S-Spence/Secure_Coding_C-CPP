...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    /* This method applies input validation to check if an SQL injection attack was attempted. */
    /* A better method would have used parameterized queries or prepared statements. However, the assignment would not allow us to edit
       the function that assembles the query.*/
    int previous_index = 0;
    int next_index = 0;

    // This loop checks the query for all occurences of an equal sign and compares the values before and after it for equivalence. 
    for (int i = 0; i < sql.length(); ++i) {
        if (sql[i] == '=') {
            previous_index = i - 1;
            next_index = i + 1;
            // Check if the values on either side of the equal sign are equivalent. 
...
void run_queries(sqlite3* db)
{
    ...
    std::cout << " " << std::endl;
    std::cout << "Testing valid queries" << std::endl;
    std::cout << "---------------------" << std::endl;
    ...
    if (!run_query(db, sql, records)) return;
    std::cout << " " << std::endl;
    std::cout << "Testing another valid query..." << std::endl;
    std::cout << "------------------------------" << std::endl;
    dump_results(sql, records);

    //  run query 1 with injection 5 times
    std::cout << " " << std::endl;
    std::cout << "Testing 5 invalid queries..." << std::endl;
    std::cout << "----------------------------" << std::endl;
    for (auto i = 0; i < 5; ++i)
    {
